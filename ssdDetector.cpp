...
void Detector::SetMean(const string& mean_file, const string& mean_value) {
  ...
  std::cout<<"mean_file.empty()="<<mean_file.empty()<<", mean_value.empty()="<<mean_value.empty()<<std::endl;
  if (!mean_file.empty()) {
  ...
  while (infile >> file) {
    if (file_type == "image") {
      ...
      for (int i = 0; i < detections.size(); ++i) {
        ...
        Point start, stop;
        start.x =  d[3] * img.cols;
        start.y =   d[4] * img.rows;
        stop.x =  d[5] * img.cols;
        stop.y =   d[6] * img.rows;
        cv::rectangle(img, start, stop, cv::Scalar(255, 0, 0), 2);
        std::stringstream scoreS;
        scoreS << score;
        std::string lable = classes[static_cast<int>(d[1])]+":"+scoreS.str()+"%";
